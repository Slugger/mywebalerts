...
def listings = tvmClient.getLineup('6340').getListingsQuickly(new StartDateOption(start), new EndDateOption(new Date(start.time + 8 * 3600000)))
def premieres = listings.findAll {
    it.showName != 'Movie' && (it.seriesPremiere || it.seasonPremiere)
}
[desc: 'Today\'s Premieres', airings: premieres, tmpl: 'airings.tmpl', size: premieres?.size(), subject: 'Today\'s TV Premieres']
